# Contributing to SBT Openapi codegen plugin

The SBT Openapi codegen plugin project welcomes contributions from anybody wishing to participate. All code or documentation that is provided must be licensed with the same license that SBT Openapi codegen plugin is licensed with (Apache 2.0, see [LICENSE](./LICENSE.md)).

## Code of Conduct

People are expected to follow the [Scala Code of Conduct](./CODE_OF_CONDUCT.md) when discussing SBT Openapi codegen plugin on the Github page, Gitter channel, or other venues.
...
```
Copyright (c) $today.year the SBT Openapi codegen plugin contributors.
